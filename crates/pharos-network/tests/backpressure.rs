...
fn install_global_warn_counter() -> Arc<AtomicUsize> {
    STALL_WARN_COUNT
        .get_or_init(|| {
            ...
            // Install a global tracing dispatcher that includes our layer.
            let subscriber = tracing_subscriber::registry().with(StallWarnLayer);
            // `set_global_default` fails only if another global was already
            // set; that is fine — the `OnceLock` prevents us from entering
            // this closure twice, so the first call always wins.

...
use crate::common::{CapturingScorer, NetworkEvent, TestHost, spawn_node, spawn_node_with_scorer};
use libp2p::PeerId;
...
async fn connect_and_wait(
    node_a: &mut crate::common::TestNode,
    node_b: &mut crate::common::TestNode,
) {
    node_a
        .handle
        ...
        RpcResponse::BlocksByRange(blocks) => {
            ...
            for (i, block) in blocks.iter().enumerate() {
                let slot = match block {
                    ...
                    MainnetSignedBeaconBlock::Fulu(inner) => inner.message.slot.0,
                };

                ...
                ("electra", "mainnet") => {
                    ...
                    Box::new(move || {
                        match run_electra_rewards_case::<E, MainnetBeaconState>(
                            ...
                            &case_name,
                            |s, fi| {
                                let d = electra_state_to_deneb(s);
...
/// Electra rewards: the flag/inactivity reward algorithm is unchanged from
/// deneb, so the electra state is projected to deneb (`electra_state_to_deneb`)
/// and the deneb-style delta closures run on the projection.
fn run_electra_rewards_case<E, S>(
    case_dir: &Path,
    case_name: &str,
    get_flag_deltas: impl Fn(&S, usize) -> (Vec<Gwei>, Vec<Gwei>),
    get_inactivity_deltas: impl Fn(&S) -> (Vec<Gwei>, Vec<Gwei>),

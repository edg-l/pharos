//! Backward state backfill — historical state reconstruction.
//!
//! Forward BLOCK backfill (`backfill.rs::run_backfill_loop`)
//! walks UP from the anchor toward the wall clock and stops at the tip; it never
//! walks below the anchor. The hot/cold freezer only writes restore
...
//!      `StateRegenService` replay primitives (`D-replay-on-read`,
...
//!      — a mismatch is a consensus bug and aborts the loop;
//!   4. persists the validated state to the cold `restore-points` /
//!      `cold-states` CFs via the existing `Store` methods.
//!
...
pub enum BackwardBackfillError {
    ...
    /// silently persisting a wrong state.
    #[error(
...
/// Backward state-backfill loop (2.3/2.6).
///
...
/// the lowest stored state is present, or when shutdown fires.
    ...
    E: ReplayBounds,
{
    ...
    // forward state we currently hold (the head). The freezer already
    ...
    loop {
        ...
        if target == 0 {
            ...
            // already held. All genesis-ward restore points are present.
        ...
        // ── Gate on the forward-backfill progress signal ──────────────
        ...
        // ── Reconstruct the state at `target_slot` ────────────────────
        ...
        // ── Validate against the block's stored state_root ────────────
        ...
        // ── Persist to the cold restore-point CFs ─────────────────────
        ...
        // Coarse progress logging: one INFO line per completed interval.
        info!(
            slot = %target_slot,
            ?state_root,

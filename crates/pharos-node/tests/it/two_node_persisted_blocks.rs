...
use crate::common::node::{build_host, spawn_node};
...
fn populate_store(
    ...
    for (root, block) in blocks {
        let slot = match block {
            ...
            MinimalSignedBeaconBlock::Fulu(inner) => inner.message.slot,
        };
...
async fn persisted_blocks_survive_restart() {
    ...
    for (i, blk) in received_blocks.iter().enumerate() {
        let slot = match blk {
            ...
            MinimalSignedBeaconBlock::Electra(inner) => inner.message.slot.0,
            MinimalSignedBeaconBlock::Fulu(inner) => inner.message.slot.0,
        };
        assert_eq!(
            slot,
            (i as u64) + 1,

...
//!
//! Because the global Prometheus recorder can only be installed once per
//! process, this file installs it via a [`OnceLock`] and then spins up
//! two independent HTTP servers (each on its own ephemeral port) backed by
//! the same handle but with different sync-state probes.
//!
...
use pharos_utils::metrics::{PrometheusHandle, SyncState, init_metrics_with_handle};

/// Returns the global `PrometheusHandle`, installing the recorder on the first
/// call.  Returns `None` if the recorder was already installed by another test
/// binary in the same process (should not happen in normal `cargo test` runs).
static HANDLE: OnceLock<Option<PrometheusHandle>> = OnceLock::new();

fn get_or_init_handle() -> Option<&'static PrometheusHandle> {
    HANDLE
        .get_or_init(|| init_metrics_with_handle().ok())
        .as_ref()
}
...
async fn start_server(port: u16, probe: Arc<dyn Fn() -> SyncState + Send + Sync>) -> String {
    let handle = get_or_init_handle()
        .expect("recorder installation failed")
        .clone();

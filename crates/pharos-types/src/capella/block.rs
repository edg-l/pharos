//! Capella `BeaconBlock` and `SignedBeaconBlock` containers.
//!
//! Mirrors the Bellatrix shape but uses the Capella `BeaconBlockBody`
//! (which adds `execution_payload.withdrawals` and `bls_to_execution_changes`).
//! Per `specs/capella/beacon-chain.md`.

use pharos_ssz::{Decode, Encode, TreeHash};
use pharos_utils::BLSSignature;

use crate::capella::body::BeaconBlockBody;
use crate::phase0::primitives::{Root, Slot, ValidatorIndex};
use crate::views::{BeaconBlockView, SignedBeaconBlockView};

// ── BeaconBlock ───────────────────────────────────────────────────────────────

/// Capella `BeaconBlock` — same envelope as Bellatrix but body is Capella.
...
/// Capella `SignedBeaconBlock` — same envelope as Bellatrix but message is Capella.
#[derive(Encode, Decode, TreeHash, Clone, Debug, PartialEq, Eq, Default)]

//! Capella `BeaconState` container.
//!
//! Per `specs/capella/beacon-chain.md` (Modified containers → BeaconState).
//!
//! Changes from Bellatrix:
//! - `latest_execution_payload_header` re-typed to `capella::ExecutionPayloadHeader`
//!   (adds `withdrawals_root`).
//! - `next_withdrawal_index: WithdrawalIndex` added [New in Capella].
//! - `next_withdrawal_validator_index: ValidatorIndex` added [New in Capella].
//! - `historical_summaries: List[HistoricalSummary, HISTORICAL_ROOTS_LIMIT]` added
//!   [New in Capella]. Stays `Naive` backend (appended rarely, ~once per 8192 slots).
//! - `historical_roots` retained but frozen (never appended after Capella upgrade).
...
use crate::capella::execution_payload::ExecutionPayloadHeader;
...
/// Capella `BeaconState` per `specs/capella/beacon-chain.md`.
///
...
> {
    ...
    /// (re-typed from bellatrix, adds `withdrawals_root`).
    ...
    /// — [New in Capella]. Stays `Naive` backend (appended rarely).
    ...
    Hash256: Default + Clone,
{
    ...
    ///
    /// Same set as bellatrix: `block_roots`, `state_roots`, `historical_roots`,
    /// `validators`, `randao_mixes`. `historical_summaries` stays Naive
    /// (appended rarely, per `D-historical-summaries-field`).
...
mod tests {
    ...
    #[test]
    fn historical_summaries_stays_naive() {
        // historical_summaries is Naive (appended rarely, per D-historical-summaries-field).
        assert!(!state().historical_summaries.backend_is_tree());
    }
}

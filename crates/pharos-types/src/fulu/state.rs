//! Fulu `BeaconState` container.
//!
//! Per `specs/fulu/beacon-chain.md` (Modified containers → BeaconState).
//!
//! ## Changes from Electra (EIP-7917)
//! - New field `proposer_lookahead: Vector[ValidatorIndex, LOOKAHEAD_WINDOW]`
//!   where `LOOKAHEAD_WINDOW = (MIN_SEED_LOOKAHEAD + 1) * SLOTS_PER_EPOCH`.
//!
//! All other Electra fields are retained unchanged.
...
/// Fulu `BeaconState` per `specs/fulu/beacon-chain.md`.
///
/// Const parameters, in order (1-13 identical to electra, plus 14 for fulu):
...
/// 14. `LOOKAHEAD_WINDOW` — derived: `(MIN_SEED_LOOKAHEAD + 1) * SLOTS_PER_EPOCH`
///     (EIP-7917)
...
> {
    ...
    // ── Fulu-only fields (EIP-7917) ───────────────────────────────────────────
    /// `proposer_lookahead: Vector[ValidatorIndex, LOOKAHEAD_WINDOW]` (EIP-7917).
    ///
    /// `LOOKAHEAD_WINDOW = (MIN_SEED_LOOKAHEAD + 1) * SLOTS_PER_EPOCH`.
    /// `proposer_lookahead[0]` is the validator index for the first proposer in
    /// the current epoch, `proposer_lookahead[1]` is the next, and so on.
    pub proposer_lookahead: SszVector<ValidatorIndex, LOOKAHEAD_WINDOW>,
    ...
    Hash256: Default + Clone,
{
    ...
    fn from_ssz_bytes(bytes: &[u8]) -> Result<Self, SszError> {
        ...
        // Fulu field
        decoder.register_type::<SszVector<ValidatorIndex, LOOKAHEAD_WINDOW>>()?;
        ...
        // Fulu field
        let proposer_lookahead: SszVector<ValidatorIndex, LOOKAHEAD_WINDOW> =
            decoder.decode_next::<SszVector<ValidatorIndex, LOOKAHEAD_WINDOW>>()?;
...
/// Mainnet fulu `BeaconState`.
///
/// `LOOKAHEAD_WINDOW = (MIN_SEED_LOOKAHEAD + 1) * SLOTS_PER_EPOCH = 2 * 32 = 64`.
...
/// Minimal fulu `BeaconState`.
///
/// `LOOKAHEAD_WINDOW = (MIN_SEED_LOOKAHEAD + 1) * SLOTS_PER_EPOCH = 2 * 8 = 16`.

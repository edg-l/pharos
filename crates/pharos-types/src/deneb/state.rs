//! Deneb `BeaconState` container.
//!
//! Per `specs/deneb/beacon-chain.md` (Modified containers → BeaconState).
//!
//! Changes from Capella:
//! - `latest_execution_payload_header` re-typed to `deneb::ExecutionPayloadHeader`
//!   (adds `blob_gas_used` and `excess_blob_gas`).
//! - All other fields identical to Capella.
...
/// Deneb `BeaconState` per `specs/deneb/beacon-chain.md`.
///
    ...
    Hash256: Default + Clone,
{
    ...
    ///
    /// Same set as capella: `block_roots`, `state_roots`, `historical_roots`,

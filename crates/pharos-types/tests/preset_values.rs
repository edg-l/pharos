//! Preset constant assertion tests for `MainnetEthSpec` and `MinimalEthSpec`.
//!
...
//!
//! If any value here fails, it indicates a typo in the `EthSpec` impl.
...
fn mainnet_max_effective_balance() {
    ...
    assert_eq!(MainnetEthSpec::MAX_EFFECTIVE_BALANCE, 32_000_000_000);
}
...
fn mainnet_eth1_data_votes_limit() {
    ...
    assert_eq!(MainnetEthSpec::ETH1_DATA_VOTES_LIMIT, 2048);
}

#[test]
fn mainnet_max_committees_per_slot() {
    // `presets/mainnet/phase0.yaml:6`
    assert_eq!(MainnetEthSpec::MAX_COMMITTEES_PER_SLOT, 64);
}

#[test]
fn mainnet_target_committee_size() {
    // `presets/mainnet/phase0.yaml:8`
    assert_eq!(MainnetEthSpec::TARGET_COMMITTEE_SIZE, 128);
}
...
fn mainnet_max_pending_attestations() {
    ...
    assert_eq!(MainnetEthSpec::MAX_PENDING_ATTESTATIONS, 4096);
}
...
fn mainnet_deposit_proof_length() {
    ...
    // `specs/phase0/beacon-chain.md:194`
    assert_eq!(MainnetEthSpec::DEPOSIT_PROOF_LENGTH, 33);
}
...
fn minimal_max_effective_balance() {
    ...
    assert_eq!(MinimalEthSpec::MAX_EFFECTIVE_BALANCE, 32_000_000_000);
}
...
fn minimal_eth1_data_votes_limit() {
    ...
    assert_eq!(MinimalEthSpec::ETH1_DATA_VOTES_LIMIT, 32);
}
...
fn minimal_epochs_per_eth1_voting_period() {
    ...
    assert_eq!(MinimalEthSpec::EPOCHS_PER_ETH1_VOTING_PERIOD, 4);
}
...
fn minimal_target_committee_size() {
    ...
    assert_eq!(MinimalEthSpec::TARGET_COMMITTEE_SIZE, 4);
}

#[test]
fn minimal_max_validators_per_committee() {
    // `presets/minimal/phase0.yaml:10`
    assert_eq!(MinimalEthSpec::MAX_VALIDATORS_PER_COMMITTEE, 2048);
}

#[test]
fn minimal_shuffle_round_count() {
    // `presets/minimal/phase0.yaml:12`
    assert_eq!(MinimalEthSpec::SHUFFLE_ROUND_COUNT, 10);
}

#[test]
fn minimal_slots_per_historical_root() {
    // `presets/minimal/phase0.yaml:42`
    assert_eq!(MinimalEthSpec::SLOTS_PER_HISTORICAL_ROOT, 64);
}
...
fn minimal_inactivity_penalty_quotient() {
    ...
    assert_eq!(MinimalEthSpec::INACTIVITY_PENALTY_QUOTIENT, 33_554_432);
}

#[test]
fn minimal_min_slashing_penalty_quotient() {
    // `presets/minimal/phase0.yaml:68`
    assert_eq!(MinimalEthSpec::MIN_SLASHING_PENALTY_QUOTIENT, 64);
}

#[test]
fn minimal_proportional_slashing_multiplier() {
    // `presets/minimal/phase0.yaml:70`
    assert_eq!(MinimalEthSpec::PROPORTIONAL_SLASHING_MULTIPLIER, 2);
}
...
fn minimal_max_pending_attestations() {
    ...
    assert_eq!(MinimalEthSpec::MAX_PENDING_ATTESTATIONS, 1024);
}
...
fn minimal_deposit_proof_length() {
    ...
    assert_eq!(MinimalEthSpec::DEPOSIT_PROOF_LENGTH, 33);
}

#[test]
fn minimal_justification_bits_length() {
    // `specs/phase0/beacon-chain.md:195` — same for all presets
    assert_eq!(MinimalEthSpec::JUSTIFICATION_BITS_LENGTH, 4);
}

#[test]
fn minimal_name() {
    assert_eq!(MinimalEthSpec::name(), "minimal");
}

#[test]
fn mainnet_name() {
    assert_eq!(MainnetEthSpec::name(), "mainnet");
}

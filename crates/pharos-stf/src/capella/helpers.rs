...
/// Copy by reference from a `bellatrix::BeaconState` into a `capella::BeaconState`.
#[allow(dead_code)]
pub(crate) fn update_capella_from_bellatrix_ref<
    const SLOTS_PER_HISTORICAL_ROOT: u64,
    const HISTORICAL_ROOTS_LIMIT: u64,
    const ETH1_DATA_VOTES_LIMIT: u64,
    const VALIDATOR_REGISTRY_LIMIT: u64,
...
>(
    ...
    state.genesis_validators_root = bel.genesis_validators_root;
    state.slot = bel.slot;
    state.fork = bel.fork.clone();
    state.latest_block_header = bel.latest_block_header.clone();
    state.block_roots = bel.block_roots.clone();
    state.state_roots = bel.state_roots.clone();
    state.historical_roots = bel.historical_roots.clone();
    state.eth1_data = bel.eth1_data.clone();
    state.eth1_data_votes = bel.eth1_data_votes.clone();
    state.eth1_deposit_index = bel.eth1_deposit_index;
    state.validators = bel.validators.clone();
    state.balances = bel.balances.clone();
    state.randao_mixes = bel.randao_mixes.clone();
    state.slashings = bel.slashings.clone();
    state.previous_epoch_participation = bel.previous_epoch_participation.clone();
    state.current_epoch_participation = bel.current_epoch_participation.clone();
    state.justification_bits = bel.justification_bits.clone();
    state.previous_justified_checkpoint = bel.previous_justified_checkpoint.clone();
    state.current_justified_checkpoint = bel.current_justified_checkpoint.clone();
    state.finalized_checkpoint = bel.finalized_checkpoint.clone();
    state.inactivity_scores = bel.inactivity_scores.clone();
    state.current_sync_committee = bel.current_sync_committee.clone();
    state.next_sync_committee = bel.next_sync_committee.clone();
}

//! Capella block-processing operations.
//!
...
pub use attester_slashing::process_attester_slashing_capella;
pub use bls_to_execution_change::process_bls_to_execution_change;
pub use execution_payload::process_execution_payload;
pub use proposer_slashing::process_proposer_slashing_capella;
...
/// `process_operations` for Capella.
///
/// Per `specs/capella/beacon-chain.md` Block processing → Modified
/// `process_operations`.
///
/// Sequence:
/// 1. Proposer slashings — Capella handler (uses `MIN_SLASHING_PENALTY_QUOTIENT_BELLATRIX`).
/// 2. Attester slashings — Capella handler.
...
/// 6. BLS-to-execution changes — [New in Capella].
#[allow(clippy::too_many_arguments)]
pub fn process_operations_capella<
...
>(
    ...
    >,
    body: &pharos_types::capella::BeaconBlockBody<
    ...
    pharos_utils::BLSPubkey: Default + Clone,
{
    ...
    // Step 1: Proposer slashings.
    for slashing in body.proposer_slashings() {
        process_proposer_slashing_capella::<
            SLOTS_PER_HISTORICAL_ROOT,
            HISTORICAL_ROOTS_LIMIT,
            ETH1_DATA_VOTES_LIMIT,
            VALIDATOR_REGISTRY_LIMIT,
    ...
    for slashing in body.attester_slashings() {
        process_attester_slashing_capella::<
            SLOTS_PER_HISTORICAL_ROOT,
            HISTORICAL_ROOTS_LIMIT,
            ETH1_DATA_VOTES_LIMIT,
            VALIDATOR_REGISTRY_LIMIT,
    ...
    // Steps 3–5: attestations, deposits, voluntary exits via altair projection.
    ...
    // Step 6: BLS-to-execution changes — [New in Capella].
    for signed_change in body.bls_to_execution_changes.as_slice() {
        process_bls_to_execution_change::<
            ...
            MAX_EXTRA_DATA_BYTES,
            E,
        >(state, signed_change, verify_signatures)?;
    }

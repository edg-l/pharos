//! `process_attester_slashing` for Capella.
//!
//! Identical logic to Bellatrix; calls `slash_validator_capella`.
...
/// `process_attester_slashing` for Capella.
pub fn process_attester_slashing_capella<
    ...
    >,
{
    ...
    if !is_valid_indexed_attestation_capella(state, att1, verify_signatures) {
        return Err(StateTransitionError::InvalidAttesterSlashing {
            reason: AttesterSlashingInvalidReason::InvalidIndexedAttestation,
        });
    }
    if !is_valid_indexed_attestation_capella(state, att2, verify_signatures) {
        return Err(StateTransitionError::InvalidAttesterSlashing {
            reason: AttesterSlashingInvalidReason::InvalidIndexedAttestation,
        });
    }
...
fn is_valid_indexed_attestation_capella<
    const SLOTS_PER_HISTORICAL_ROOT: u64,
    const HISTORICAL_ROOTS_LIMIT: u64,
    const ETH1_DATA_VOTES_LIMIT: u64,
    const VALIDATOR_REGISTRY_LIMIT: u64,

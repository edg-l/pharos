...
/// Copy the shared fields from a `capella::BeaconState` back into a
/// `deneb::BeaconState` (by reference).
#[allow(dead_code)]
pub(crate) fn update_deneb_from_capella_ref<
    const SLOTS_PER_HISTORICAL_ROOT: u64,
    const HISTORICAL_ROOTS_LIMIT: u64,
    const ETH1_DATA_VOTES_LIMIT: u64,
    const VALIDATOR_REGISTRY_LIMIT: u64,
...
>(
    ...
    state.genesis_validators_root = capella.genesis_validators_root;
    state.slot = capella.slot;
    state.fork = capella.fork.clone();
    state.latest_block_header = capella.latest_block_header.clone();
    state.block_roots = capella.block_roots.clone();
    state.state_roots = capella.state_roots.clone();
    state.historical_roots = capella.historical_roots.clone();
    state.eth1_data = capella.eth1_data.clone();
    state.eth1_data_votes = capella.eth1_data_votes.clone();
    state.eth1_deposit_index = capella.eth1_deposit_index;
    state.validators = capella.validators.clone();
    state.balances = capella.balances.clone();
    state.randao_mixes = capella.randao_mixes.clone();
    state.slashings = capella.slashings.clone();
    state.previous_epoch_participation = capella.previous_epoch_participation.clone();
    state.current_epoch_participation = capella.current_epoch_participation.clone();
    state.justification_bits = capella.justification_bits.clone();
    state.previous_justified_checkpoint = capella.previous_justified_checkpoint.clone();
    state.current_justified_checkpoint = capella.current_justified_checkpoint.clone();
    state.finalized_checkpoint = capella.finalized_checkpoint.clone();
    state.inactivity_scores = capella.inactivity_scores.clone();
    state.current_sync_committee = capella.current_sync_committee.clone();
    state.next_sync_committee = capella.next_sync_committee.clone();
    state.next_withdrawal_index = capella.next_withdrawal_index;
    state.next_withdrawal_validator_index = capella.next_withdrawal_validator_index;
    state.historical_summaries = capella.historical_summaries.clone();
}
...
// ── slash_validator (deneb) ───────────────────────────────────────────────────

/// `slash_validator` for Deneb.
///
/// Identical to capella: uses `MIN_SLASHING_PENALTY_QUOTIENT_BELLATRIX`.
#[allow(dead_code)]
pub(crate) fn slash_validator_deneb<
    const SLOTS_PER_HISTORICAL_ROOT: u64,
    const HISTORICAL_ROOTS_LIMIT: u64,
    const ETH1_DATA_VOTES_LIMIT: u64,
    const VALIDATOR_REGISTRY_LIMIT: u64,
    ...
    >,
{
    ...
    // Deneb inherits bellatrix's penalty quotient (same as capella).
    ...
    // Proposer reward via altair projection.
    let altair_state = deneb_state_to_altair(state);

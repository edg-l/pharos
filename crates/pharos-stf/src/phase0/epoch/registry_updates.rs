    ...
    E::BeaconState: BeaconStateWrite,
{
    ...
    let mut activation_queue: Vec<usize> = (0..state.validators().len())
        .filter(|&i| is_eligible_for_activation(finalized_epoch, &state.validators()[i]))
        .collect();

    activation_queue.sort_by_key(|&i| (state.validators()[i].activation_eligibility_epoch, i));

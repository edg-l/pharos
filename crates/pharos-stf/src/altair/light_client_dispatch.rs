...
use pharos_types::{
    ...
    deneb::{
        BeaconBlock as DenebBeaconBlock, BeaconState as DenebBeaconState,
        light_client::{
            LightClientBootstrap as DenebLCBootstrap,
            LightClientFinalityUpdate as DenebLCFinalityUpdate,
            LightClientOptimisticUpdate as DenebLCOptimisticUpdate,
...
use crate::deneb::helpers::deneb_state_to_altair;
use crate::deneb::light_client::deneb_block_to_light_client_header;
...
/// Dispatch trait for running the LC snapshot writes on deneb states.
///
/// Projects the deneb state to altair (via `deneb_state_to_altair`)
/// for the state fields, and uses a Deneb-specific body hash for the
...
pub trait DenebDispatchBounds<E: BeaconSpec>: Sized {
    ...
    fn call_update_lc_snapshots_deneb<S: pharos_storage::Store<E>>(
        ...
        attested_state: Option<&Self>,
        attested_block: Option<&E::DenebBeaconBlock>,
        finalized_block: Option<&E::DenebBeaconBlock>,
        store: &S,
    );
}
...
where
    E: BeaconSpec<
            ...
            DenebBeaconState = DenebBeaconState<
                ...
                MAX_EXTRA_DATA_BYTES,
            >,
            DenebBeaconBlock = DenebBeaconBlock<
                MAX_PROPOSER_SLASHINGS,
                MAX_ATTESTER_SLASHINGS,
                MAX_ATTESTATIONS,
                MAX_DEPOSITS,
            ...
            AltairLightClientFinalityUpdate = LightClientFinalityUpdate<SYNC_COMMITTEE_SIZE>,
            AltairLightClientOptimisticUpdate = LightClientOptimisticUpdate<SYNC_COMMITTEE_SIZE>,
            DenebLightClientBootstrap = DenebLCBootstrap<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
            DenebLightClientUpdate = DenebLCUpdate<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
            DenebLightClientFinalityUpdate = DenebLCFinalityUpdate<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
            DenebLightClientOptimisticUpdate = DenebLCOptimisticUpdate<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
    ...
    pharos_types::bellatrix::Transaction<MAX_BYTES_PER_TRANSACTION>: Default + Clone,
    pharos_types::capella::Withdrawal: Default + Clone,
    pharos_types::deneb::execution_payload::ExecutionPayloadHeader<
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >: TreeHash,
    Bytes32: Default + Clone,
{
    fn call_update_lc_snapshots_deneb<S: pharos_storage::Store<E>>(
        ...
        // Project deneb state to altair for state-based LC operations.
        let post_state_altair = deneb_state_to_altair(self);

        // Block root uses the full Deneb block hash (includes blob_kzg_commitments).
        let block_root = block.tree_hash_root();

        // 1. Store LightClientBootstrap for this block.
        if let Some(bootstrap) = create_lc_bootstrap_deneb::<
            MAX_PROPOSER_SLASHINGS,
            MAX_ATTESTER_SLASHINGS,
            MAX_ATTESTATIONS,
            MAX_DEPOSITS,
        ...
        >(&post_state_altair, block)
        {
            let _ = StoreT::put_light_client_bootstrap_deneb(store, block_root, &bootstrap);
        }
        ...
        let maybe_update = attested_state
            .zip(attested_block)
            .and_then(|(att_s, att_b)| {
                let att_s_altair = deneb_state_to_altair(att_s);
                create_lc_update_deneb::<
                    MAX_PROPOSER_SLASHINGS,
                    MAX_ATTESTER_SLASHINGS,
                    MAX_ATTESTATIONS,
                    MAX_DEPOSITS,
        ...
        if let Some(update) = maybe_update {
            let period =
                compute_sync_committee_period_at_slot::<E>(update.attested_header.beacon.slot);
            let should_store = match StoreT::get_light_client_update_deneb(store, period) {
                Ok(Some(existing)) => is_better_deneb_update::<
                    SYNC_COMMITTEE_SIZE,
                    BYTES_PER_LOGS_BLOOM,
                    MAX_EXTRA_DATA_BYTES,
                >(&update, &existing),
            ...
            if should_store {
                let _ = StoreT::put_light_client_update_deneb(store, period, &update);
            }

            if is_finality_update_deneb(&update) {
                let finality_update = create_lc_finality_update_deneb(&update);
                let _ = StoreT::put_light_client_finality_update_deneb(store, &finality_update);
            }

            let optimistic_update = create_lc_optimistic_update_deneb(&update);
            let _ = StoreT::put_light_client_optimistic_update_deneb(store, &optimistic_update);
        }
    }
}

// ── Deneb LC helper functions ─────────────────────────────────────────────────

/// Build a Deneb `LightClientBootstrap`.
#[allow(clippy::too_many_arguments)]
fn create_lc_bootstrap_deneb<
    const MAX_PROPOSER_SLASHINGS: u64,
    const MAX_ATTESTER_SLASHINGS: u64,
    const MAX_ATTESTATIONS: u64,
    const MAX_DEPOSITS: u64,
...
>(
    ...
    block: &DenebBeaconBlock<
        ...
        MAX_BLOB_COMMITMENTS_PER_BLOCK,
    >,
) -> Option<DenebLCBootstrap<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>>
where
    pharos_types::altair::BeaconBlockBody<
        MAX_PROPOSER_SLASHINGS,
        MAX_ATTESTER_SLASHINGS,
    ...
    Bytes32: Default + Clone,
{
    ...
    // Use the STF-verified block.state_root (deneb post-state includes blob_gas_used).
    let mut header = state.latest_block_header.clone();
    header.state_root = block.state_root;
    let block_root = header.tree_hash_root();
    ...
    Some(DenebLCBootstrap {
        header: deneb_block_to_light_client_header(block),
        current_sync_committee: state.current_sync_committee.clone(),
        current_sync_committee_branch,
    })
}

/// Build a Deneb `LightClientUpdate`.
#[allow(clippy::too_many_arguments)]
fn create_lc_update_deneb<
    const MAX_PROPOSER_SLASHINGS: u64,
    const MAX_ATTESTER_SLASHINGS: u64,
    const MAX_ATTESTATIONS: u64,
    const MAX_DEPOSITS: u64,
...
>(
    ...
    attested_state: &AltairBeaconState<
        ...
        SYNC_COMMITTEE_SIZE,
    >,
    attested_block: &DenebBeaconBlock<
        MAX_PROPOSER_SLASHINGS,
        MAX_ATTESTER_SLASHINGS,
        MAX_ATTESTATIONS,
        MAX_DEPOSITS,
    ...
    finalized_block: Option<
        ...
        >,
    >,
) -> Option<DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>>
where
    pharos_types::altair::BeaconBlockBody<
        MAX_PROPOSER_SLASHINGS,
        MAX_ATTESTER_SLASHINGS,
    ...
    Bytes32: Default + Clone,
{
    ...
    let (finalized_header, finality_branch) = if let Some(fin_block) = finalized_block {
        let header = if fin_block.slot.0 != 0 {
            deneb_block_to_light_client_header(fin_block)
        } else {
            Default::default()
        };
        let fin_branch_hashes = compute_state_proof(attested_state, FINALIZED_ROOT_GINDEX);
    ...
    Some(DenebLCUpdate {
        attested_header: deneb_block_to_light_client_header(attested_block),
        next_sync_committee,
        next_sync_committee_branch,
        finalized_header,
        finality_branch,
...
fn is_finality_update_deneb<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    update: &DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>,
) -> bool
where
    Bytes32: Default + Clone + PartialEq,
{
...
fn is_better_deneb_update<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    new: &DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>,
    existing: &DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>,
) -> bool
where
    Bytes32: Default + Clone + PartialEq,
{
    // Prefer updates with finality branch over those without.
    let new_has_fin = is_finality_update_deneb(new);
    let existing_has_fin = is_finality_update_deneb(existing);
    if new_has_fin != existing_has_fin {
        return new_has_fin;
    }
    // Both or neither have finality: prefer more participants.
...
fn create_lc_finality_update_deneb<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    update: &DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>,
) -> DenebLCFinalityUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>
where
    Bytes32: Default + Clone,
{
    DenebLCFinalityUpdate {
        attested_header: update.attested_header.clone(),
        finalized_header: update.finalized_header.clone(),
        finality_branch: update.finality_branch.clone(),
        sync_aggregate: update.sync_aggregate.clone(),
...
fn create_lc_optimistic_update_deneb<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    update: &DenebLCUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>,
) -> DenebLCOptimisticUpdate<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>
where
    Bytes32: Default + Clone,
{
    DenebLCOptimisticUpdate {
        attested_header: update.attested_header.clone(),
        sync_aggregate: update.sync_aggregate.clone(),
        signature_slot: update.signature_slot,
    }
...
/// Dispatch trait for running the LC snapshot writes on electra states.
///
/// Projects the electra state to altair (via `electra_state_to_altair`)
/// for the state fields, and uses an Electra-specific body hash for the
...
pub trait ElectraDispatchBounds<E: BeaconSpec>: Sized {
    ...
    fn call_update_lc_snapshots_electra<S: pharos_storage::Store<E>>(
        ...
        attested_state: Option<&Self>,
        attested_block: Option<&E::ElectraBeaconBlock>,
        finalized_block: Option<&E::ElectraBeaconBlock>,
        store: &S,
    );
}
...
where
    E: BeaconSpec<
            ...
            AltairLightClientFinalityUpdate = LightClientFinalityUpdate<SYNC_COMMITTEE_SIZE>,
            AltairLightClientOptimisticUpdate = LightClientOptimisticUpdate<SYNC_COMMITTEE_SIZE>,
            ElectraLightClientBootstrap = pharos_types::electra::light_client::LightClientBootstrap<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
            ElectraLightClientUpdate = pharos_types::electra::light_client::LightClientUpdate<
                SYNC_COMMITTEE_SIZE,
                BYTES_PER_LOGS_BLOOM,
                MAX_EXTRA_DATA_BYTES,
            >,
            ElectraLightClientFinalityUpdate =
                pharos_types::electra::light_client::LightClientFinalityUpdate<
                    SYNC_COMMITTEE_SIZE,
                    BYTES_PER_LOGS_BLOOM,
                    MAX_EXTRA_DATA_BYTES,
                >,
            ElectraLightClientOptimisticUpdate =
                pharos_types::electra::light_client::LightClientOptimisticUpdate<
                    SYNC_COMMITTEE_SIZE,
                    BYTES_PER_LOGS_BLOOM,
                    MAX_EXTRA_DATA_BYTES,
                >,
    ...
    pharos_types::bellatrix::Transaction<MAX_BYTES_PER_TRANSACTION>: Default + Clone,
    pharos_types::capella::Withdrawal: Default + Clone,
    pharos_types::electra::execution_payload::ExecutionPayloadHeader<
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >: TreeHash,
    Bytes32: Default + Clone,
{
    fn call_update_lc_snapshots_electra<S: pharos_storage::Store<E>>(
        &self,
        block: &pharos_types::electra::BeaconBlock<
            MAX_PROPOSER_SLASHINGS,
            MAX_ATTESTER_SLASHINGS_ELECTRA,
            MAX_ATTESTATIONS_ELECTRA,
            MAX_DEPOSITS,
        ...
        attested_state: Option<&Self>,
        attested_block: Option<
            &pharos_types::electra::BeaconBlock<
                MAX_PROPOSER_SLASHINGS,
                MAX_ATTESTER_SLASHINGS_ELECTRA,
                MAX_ATTESTATIONS_ELECTRA,
                MAX_DEPOSITS,
        ...
        use pharos_ssz::TreeHash as _;
        use pharos_storage::Store as StoreT;
        use pharos_types::electra::light_client::{
            CURRENT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA, NEXT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA,
            FINALITY_BRANCH_DEPTH_ELECTRA,
            LightClientBootstrap as ElectraLCBootstrap,
            LightClientFinalityUpdate as ElectraLCFinalityUpdate,
            LightClientOptimisticUpdate as ElectraLCOptimisticUpdate,
            LightClientUpdate as ElectraLCUpdate,
        };

        // Project electra state to altair for state-based LC operations.
        let post_state_altair = crate::electra::helpers::electra_state_to_altair(self);

        // Block root uses the full Electra block hash (includes execution_requests).
        let block_root = block.tree_hash_root();

        // 1. Store LightClientBootstrap for this block.
        if let Some(bootstrap) = create_lc_bootstrap_electra::<
            MAX_PROPOSER_SLASHINGS,
            MAX_ATTESTER_SLASHINGS_ELECTRA,
            MAX_ATTESTATIONS_ELECTRA,
            MAX_DEPOSITS,
        ...
        >(&post_state_altair, block)
        {
            let _ = StoreT::put_light_client_bootstrap_electra(store, block_root, &bootstrap);
        }
        ...
        let maybe_update = attested_state
            .zip(attested_block)
            .and_then(|(att_s, att_b)| {
                let att_s_altair = crate::electra::helpers::electra_state_to_altair(att_s);
                create_lc_update_electra::<
                    MAX_PROPOSER_SLASHINGS,
                    MAX_ATTESTER_SLASHINGS_ELECTRA,
                    MAX_ATTESTATIONS_ELECTRA,
                    MAX_DEPOSITS,
        ...
        if let Some(update) = maybe_update {
            let period =
                compute_sync_committee_period_at_slot::<E>(update.attested_header.beacon.slot);
            let should_store = match StoreT::get_light_client_update_electra(store, period) {
                Ok(Some(existing)) => is_better_electra_update::<
                    SYNC_COMMITTEE_SIZE,
                    BYTES_PER_LOGS_BLOOM,
                    MAX_EXTRA_DATA_BYTES,
                >(&update, &existing),
            ...
            if should_store {
                let _ = StoreT::put_light_client_update_electra(store, period, &update);
            }

            if is_finality_update_electra(&update) {
                let finality_update = ElectraLCFinalityUpdate {
                    attested_header: update.attested_header.clone(),
                    finalized_header: update.finalized_header.clone(),
                    finality_branch: update.finality_branch.clone(),
                    sync_aggregate: update.sync_aggregate.clone(),
                ...
                let _ =
                    StoreT::put_light_client_finality_update_electra(store, &finality_update);
            }

            let optimistic_update = ElectraLCOptimisticUpdate {
                attested_header: update.attested_header.clone(),
                sync_aggregate: update.sync_aggregate.clone(),
                signature_slot: update.signature_slot,
            };
            let _ =
                StoreT::put_light_client_optimistic_update_electra(store, &optimistic_update);

            let _ = (
                CURRENT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA,
                NEXT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA,
                FINALITY_BRANCH_DEPTH_ELECTRA,
                &(ElectraLCBootstrap::<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>::default),
                &(ElectraLCUpdate::<SYNC_COMMITTEE_SIZE, BYTES_PER_LOGS_BLOOM, MAX_EXTRA_DATA_BYTES>::default),
            );
        }
    }
}
...
#[allow(clippy::too_many_arguments)]
fn create_lc_bootstrap_electra<
    const MAX_PROPOSER_SLASHINGS: u64,
    const MAX_ATTESTER_SLASHINGS_ELECTRA: u64,
    const MAX_ATTESTATIONS_ELECTRA: u64,
    const MAX_DEPOSITS: u64,
...
>(
    ...
    pharos_types::electra::light_client::LightClientBootstrap<
        SYNC_COMMITTEE_SIZE,
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >,
...
{
    use pharos_ssz::TreeHash as _;
    use pharos_types::electra::light_client::CURRENT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA;

    if state.slot != state.latest_block_header.slot {
        return None;
    }

    // Use the STF-verified block.state_root (electra post-state includes execution_requests).
    let mut header = state.latest_block_header.clone();
    header.state_root = block.state_root;
    let block_root = header.tree_hash_root();
    ...
    let branch_hashes = compute_state_proof(state, CURRENT_SYNC_COMMITTEE_GINDEX_ELECTRA);
    debug_assert_eq!(
        branch_hashes.len(),
        CURRENT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA as usize
    );
    let mut branch_vec: Vec<Bytes32> = branch_hashes;
    while branch_vec.len() < CURRENT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA as usize {
        branch_vec.push(Bytes32::default());
    }
    let current_sync_committee_branch = SszVector::from_vec(branch_vec).unwrap_or_default();

    Some(pharos_types::electra::light_client::LightClientBootstrap {
        header: crate::electra::light_client::electra_block_to_light_client_header(block),
        current_sync_committee: state.current_sync_committee.clone(),
        current_sync_committee_branch,
    })
}

/// Build an Electra `LightClientUpdate`.
#[allow(clippy::too_many_arguments)]
fn create_lc_update_electra<
    const MAX_PROPOSER_SLASHINGS: u64,
    const MAX_ATTESTER_SLASHINGS_ELECTRA: u64,
    const MAX_ATTESTATIONS_ELECTRA: u64,
    const MAX_DEPOSITS: u64,
...
>(
    ...
    attested_state: &AltairBeaconState<
        ...
        SYNC_COMMITTEE_SIZE,
    >,
    attested_block: &pharos_types::electra::BeaconBlock<
        MAX_PROPOSER_SLASHINGS,
        MAX_ATTESTER_SLASHINGS_ELECTRA,
        MAX_ATTESTATIONS_ELECTRA,
        MAX_DEPOSITS,
    ...
    pharos_types::electra::light_client::LightClientUpdate<
        SYNC_COMMITTEE_SIZE,
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >,
    ...
    Bytes32: Default + Clone,
{
    use pharos_ssz::TreeHash as _;
    use pharos_types::electra::light_client::{
        FINALITY_BRANCH_DEPTH_ELECTRA, NEXT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA,
    };
    ...
    let (next_sync_committee, next_sync_committee_branch) =
        if update_attested_period == update_signature_period {
            let nsc_branch_hashes =
                compute_state_proof(attested_state, NEXT_SYNC_COMMITTEE_GINDEX_ELECTRA);
            let mut nsc_vec: Vec<Bytes32> = nsc_branch_hashes;
            while nsc_vec.len() < NEXT_SYNC_COMMITTEE_BRANCH_DEPTH_ELECTRA as usize {
                nsc_vec.push(Bytes32::default());
            }
            (
                attested_state.next_sync_committee.clone(),
    ...
    let (finalized_header, finality_branch) = if let Some(fin_block) = finalized_block {
        let header = if fin_block.slot.0 != 0 {
            crate::electra::light_client::electra_block_to_light_client_header(fin_block)
        } else {
            Default::default()
        };
        let fin_branch_hashes = compute_state_proof(attested_state, FINALIZED_ROOT_GINDEX_ELECTRA);
        let mut fin_vec: Vec<Bytes32> = fin_branch_hashes;
        while fin_vec.len() < FINALITY_BRANCH_DEPTH_ELECTRA as usize {
            fin_vec.push(Bytes32::default());
        }
        (header, SszVector::from_vec(fin_vec).unwrap_or_default())
    } else {
    ...
    Some(pharos_types::electra::light_client::LightClientUpdate {
        attested_header: crate::electra::light_client::electra_block_to_light_client_header(
            attested_block,
        ),
        next_sync_committee,
        next_sync_committee_branch,
...
fn is_finality_update_electra<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    update: &pharos_types::electra::light_client::LightClientUpdate<
        SYNC_COMMITTEE_SIZE,
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >,
...
where
    Bytes32: Default + Clone + PartialEq,
{
    use pharos_types::electra::light_client::FINALITY_BRANCH_DEPTH_ELECTRA;
    update.finality_branch.as_slice()
        != vec![Bytes32::default(); FINALITY_BRANCH_DEPTH_ELECTRA as usize].as_slice()
}

fn is_better_electra_update<
    const SYNC_COMMITTEE_SIZE: u64,
    const BYTES_PER_LOGS_BLOOM: u64,
    const MAX_EXTRA_DATA_BYTES: u64,
>(
    new: &pharos_types::electra::light_client::LightClientUpdate<
        SYNC_COMMITTEE_SIZE,
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >,
    existing: &pharos_types::electra::light_client::LightClientUpdate<
        SYNC_COMMITTEE_SIZE,
        BYTES_PER_LOGS_BLOOM,
        MAX_EXTRA_DATA_BYTES,
    >,
...
{
    // Prefer updates with finality branch over those without.
    let new_has_fin = is_finality_update_electra(new);
    let existing_has_fin = is_finality_update_electra(existing);
    if new_has_fin != existing_has_fin {
        return new_has_fin;
    }
    // Both or neither have finality: prefer more participants.

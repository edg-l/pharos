...
//!   `blob-sidecars` CF sentinel) returns `SchemaMismatch { found: 3, expected: 4 }`.
...
/// CF was added) must return `SchemaMismatch { found: 3, expected: 4 }`.
///
...
fn schema_v3_db_returns_schema_mismatch() {
    ...
    assert!(
        ...
        "expected SchemaMismatch{{found:3,expected:4}}, got {result:?}"
    );
}
